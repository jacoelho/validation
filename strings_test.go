...
func TestStringsAllowed(t *testing.T) {
	rule := validation.StringsAllowed("admin", "user", "guest")
	...
	tests := []struct {
		...
		{
			name:    "allowed value 'admin' should pass",
			value:   "admin",
			wantErr: false,
		},
		{
			name:    "allowed value 'user' should pass",
			value:   "user",
			wantErr: false,
		},
		{
			name:    "allowed value 'guest' should pass",
			value:   "guest",
			wantErr: false,
		},
		{
...
func TestStringsDisallowed(t *testing.T) {
	rule := validation.StringsDisallowed("root", "admin", "administrator")
	...
	tests := []struct {
		...
		{
			name:    "allowed value should pass",
			value:   "user",
			wantErr: false,
		},
		{
			name:    "disallowed value 'root' should fail",
			value:   "root",
			wantErr: true,
			errCode: "disallowed",
		},
		{
			name:    "disallowed value 'admin' should fail",
			value:   "admin",
			wantErr: true,
			errCode: "disallowed",
		},
		{
			name:    "disallowed value 'administrator' should fail",
			value:   "administrator",
			wantErr: true,
			errCode: "disallowed",
		},
		...
		{
			name:    "case sensitive check should pass",
			value:   "Admin",
			wantErr: false,
		},
		{
			name:    "similar but not exact match should pass",
...
func TestStringsErrorParams(t *testing.T) {
	...
	t.Run("StringsAllowed error params", func(t *testing.T) {
		rule := validation.StringsAllowed("a", "b", "c")
		err := rule("d")
		...
		if err.Params["value"] != "d" {
			t.Errorf("expected value param to be 'd', got %v", err.Params["value"])
		}
	})
}
